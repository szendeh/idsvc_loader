...
"""
idsvc bulk loader, a fork of LibQA
Takes an arbitrary number of files that contain tab-delimited lines delimited, in turn, by \n, and builds POST requests for each.
POST requests that return something other than 200 are logged and collected into a single failed file that can be re-tested.
